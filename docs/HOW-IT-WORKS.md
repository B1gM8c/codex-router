...
seconds so client retries do not repeatedly hit native quota. The short backoff
stores only the already-hashed cache key and an expiry timestamp; other
accounts and delegated payloads remain independent. Operators can tune the
window with `CODEX_ROUTER_AGENT_RELAY_FAILURE_BACKOFF_MS`.
